...
const reconcileData = (newList) => {
  ...
  newList.forEach((newItem) => {
    ...
    // Add operation
    if (!Array.from(listElement.children).some((row) => id === +row.id)) {
      const row = document.createElement("div");
      row.id = id;
      ...
      Array.from(listElement.children).forEach((row) => {
        if (!newList.some((newItem) => newItem.id === +row.id)) {
          listElement.removeChild(document.getElementById(row.id));
        }
        // Update operation
        else if (+row.id === id) {
