...
try {
  ADMINS = JSON.parse(
    fs.readFileSync(`${__dirname}/admins.json`, { encoding: "utf-8" })
  );
  USERS = JSON.parse(
    fs.readFileSync(`${__dirname}/users.json`, { encoding: "utf-8" })
  );
  COURSES = JSON.parse(
    fs.readFileSync(`${__dirname}/courses.json`, { encoding: "utf-8" })
  );
} catch {
  ADMINS = [];
  USERS = [];
...
app.post("/admin/signup", (req, res) => {
  ...
  if (admin) {
    ...
    fs.writeFileSync(`${__dirname}/admins.json`, JSON.stringify(ADMINS));
...
app.post("/admin/courses", authenticateAdmin, (req, res) => {
  ...
  fs.writeFileSync(`${__dirname}/courses.json`, JSON.stringify(COURSES));
  res.json({
...
app.post("/users/signup", (req, res) => {
  ...
  if (exists) {
    ...
    const token = generateToken(newUser, UserSecret);
