function createDomElements(data) {
	...
	data.forEach(function (item) {
		...
		if (existingChild) {
			...
			existingChild.children[0].innerHTML = `${item.id} ${item.title}`;
			existingChild.children[1].innerHTML = ` ${item.description} updated`;
			...
			grandChildElement1.innerHTML = `${item.id} ${item.title}`;
