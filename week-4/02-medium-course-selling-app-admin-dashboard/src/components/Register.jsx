...
function Register() {
  ...
  const register = async (e) => {
    ...
    try {
      ...
      //   setStatus('now please login with same username !');
      //   setInterval(() => {
      //     navigate('/login');
      //   }, 4000);
      // }, 2000);
  ...
  return (
    <div className="h-[100vh] flex flex-col justify-center items-center bg-gray-300">
      ...
      <div className="flex flex-col space-y-3 p-10 rounded-md shadow-xd bg-white text-black">
        {status && <p className="text-red-600 font-semibold text-[0.9rem] text-center">{status}</p>}
        <input
          type={"text"}
        ...
        <div>
          <button onClick={(e)=> register(e)} className="text-white bg-green-600 px-4 py-1 rounded-md">
            Register
          </button>
        </div>
        ...
        <p className="text-gray-400">
          already registered ?{" "}
          <a onClick={()=> navigate('/login')} className="cursor-pointer hover:text-blue-900 text-blue-600">
            Login
          </a>
        </p>
      </div>
